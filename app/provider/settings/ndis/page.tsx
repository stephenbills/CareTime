...
export default function NdisLineItemsPage() {
  ...
  async function handleSave() {
    ...
    if (editingId) {
      const { error: err } = await supabase
        .from('ndis_line_items').update(payload).eq('id', editingId)
      if (err) { setError(err.message); setSaving(false); return }
    } else {
      const { error: err } = await supabase
        .from('ndis_line_items').insert(payload)
      if (err) { setError(err.message); setSaving(false); return }
    }
  ...
  const visibleItems = items.filter(i => showInactive ? true : i.active)
  ...
  if (!providerId) return (
    <div className="p-8">
      <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-4 py-3 text-sm">
        Please save your Provider Settings first before adding NDIS line items.{' '}
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">NDIS Support Catalogue</h1>
          <p className="text-gray-500 text-sm mt-0.5">
            Manage NDIS line items used when scheduling activities
          </p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-blue-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-1">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Line Item Number <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
            ...
            <div className="col-span-1">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Unit Price ($ per hour) <span className="text-red-500">*</span>
              </label>
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="e.g. Assistance with Daily Life - Standard - Weekday Daytime"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            >
              <Check size={14} /> {saving ? 'Saving…' : editingId ? 'Save Changes' : 'Add Line Item'}
            </button>
            <button
              onClick={closeForm}
      ...
      {visibleItems.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          <p className="text-gray-400 text-sm mb-3">No NDIS line items yet.</p>
          <button
            onClick={openNew}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-50">
                    <th className="px-5 py-2.5 text-left text-xs font-medium text-gray-400">Line Item</th>
                    <th className="px-5 py-2.5 text-left text-xs font-medium text-gray-400">Description</th>
                    <th className="px-5 py-2.5 text-right text-xs font-medium text-gray-400">Unit Price</th>
                    <th className="px-5 py-2.5 text-center text-xs font-medium text-gray-400">Status</th>
                    <th className="px-5 py-2.5 text-right text-xs font-medium text-gray-400">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {catItems.map((item, i) => (
                    <tr
                      key={item.id}
                      className={`${i < catItems.length - 1 ? 'border-b border-gray-50' : ''} ${!item.active ? 'opacity-50' : ''}`}
                    >
      ...
      {items.length > 0 && (
        <p className="text-xs text-gray-400 mt-4">
          {items.filter(i => i.active).length} active line item{items.filter(i => i.active).length !== 1 ? 's' : ''}
          {items.filter(i => !i.active).length > 0 && `, ${items.filter(i => !i.active).length} inactive`}
        </p>
      )}
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <h3 className="font-semibold text-gray-900 mb-2">Delete line item?</h3>
            <p className="text-sm text-gray-500 mb-5">
              This will permanently delete this line item. Activities that already use it will not be affected.
            </p>
