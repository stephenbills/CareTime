...
export default function CalendarPage() {
    ...
    async function load() {
      ...
      const [{ data: acts }, { data: cls }, { data: crs }] = await Promise.all([
        supabase.from('activities').select('*')
          ...
          .order('start_time', { ascending: true }), // ← sorted by time
  ...
  function activitiesForDay(day: number) {
    // Already sorted by start_time from the query
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{activities.length} activit{activities.length !== 1 ? 'ies' : 'y'} this month</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              const dayActs = activitiesForDay(day) // already time-sorted
              return (
                <div key={day} onClick={() => setSelectedDay(day)}
                  ...
                  {/* Show up to 6 activities in time order */}
                  <div className="space-y-0.5">
                    {dayActs.slice(0, 6).map(a => (
                      <div key={a.id}
                        onClick={e => { e.stopPropagation(); router.push(`/provider/activities/${a.id}`) }}
                        className={`text-xs px-1 py-0.5 rounded border truncate cursor-pointer hover:opacity-80 transition-opacity
                          ${STATUS_COLORS[a.status] || 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                        {formatTime(a.start_time)} {a.title}
                      </div>
                    ))}
                    {dayActs.length > 6 && (
                      <div className="text-xs text-gray-400 px-1">+{dayActs.length - 6} more</div>
                    )}
                  </div>
                </div>
              )
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          {selectedActivities.length === 0 ? (
            ...
            // Detail panel also sorted — already sorted by query
              ...
              {selectedActivities.map(a => (
                <Link key={a.id} href={`/provider/activities/${a.id}`}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full border whitespace-nowrap flex-shrink-0
